...
use sea_orm::{
    sea_query::Query, ColumnTrait, Condition, DatabaseConnection, EntityTrait, QueryFilter,
};
...
pub async fn get_data_scope_user_ids(
    ...
    let (user_id, dept_id, role_id, data_scope) = match u_s {
        None => return Err(anyhow!("用户不存在")),
        Some(x) => {
            let r_s = sys_role::Entity::find()
                .filter(sys_role::Column::RoleId.eq(x.role_id.clone()))
                .one(db)
                .await?;
            match r_s {
