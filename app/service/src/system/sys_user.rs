...
use crate::{service_utils, service_utils::jwt::KEYS};
    ...
    let user = match user_s {
        ...
        Some(u) => {
            let dept_s = sys_dept::Entity::find()
                .filter(sys_dept::Column::DeptId.eq(u.dept_id.clone()))
                .one(db)
                .await?;
            match dept_s {
...
pub async fn add(db: &DatabaseConnection, req: SysUserAddReq, c_user_id: String) -> Result<String> {
    ...
    let salt = utils::rand_utils::rand_s(10);
    let passwd = utils::rand_utils::encrypt_password(&req.user_password, &salt);
...
pub async fn reset_passwd(db: &DatabaseConnection, req: ResetPwdReq) -> Result<String> {
    let salt = utils::rand_utils::rand_s(10);
    let passwd = utils::rand_utils::encrypt_password(&req.new_passwd, &salt);
    let now: NaiveDateTime = Local::now().naive_local();
    let txn = db.begin().await?;
    // 更新用户信息
    SysUser::update_many()
...
pub async fn update_passwd(
        ...
        .await?
    {
        None => return Err(anyhow!("用户不存在")),
        Some(x) => {
            let pwd = utils::rand_utils::encrypt_password(&req.old_passwd, &x.user_salt);
...
pub async fn login(
    ...
    // 验证验证码
    if utils::rand_utils::encrypt_password(&login_req.code, &login_req.code) != login_req.uuid {
        msg = "验证码错误".to_string();
        status = "0".to_string();
        set_login_info(
            header,
    ...
    //  验证密码是否正确
    if utils::rand_utils::encrypt_password(&login_req.user_password, &user.user_salt)
        != user.user_password
    {
        msg = "密码错误".to_string();
        status = "0".to_string();
    ...
    let claims = AuthPayload {
        user_id: user.id.clone(),          // 用户id
        name: login_req.user_name.clone(), // 用户名
    };
    let token_id = scru128::new_string();
    let token = authorize(claims.clone(), token_id.clone()).await;
    // 成功登录后
    //  写入登录日志
...
pub async fn fresh_token(user: &Claims) -> Result<AuthBody> {
    // 注册JWT
    let claims = AuthPayload {
        user_id: user.clone().user_id, // 用户id
        name: user.clone().name,       // 用户名
    };
    let token = authorize(claims.clone(), user.clone().token_id).await;
    // 成功登录后
    // 更新原始在线日志
...
pub async fn set_login_info(
    ...
    let u = service_utils::web_utils::get_client_info(header, remote_ip).await;
    // 写入登录日志
    let u2 = u.clone();
...
pub async fn authorize(payload: AuthPayload, token_id: String) -> AuthBody {
    let iat = Local::now();
    let exp = iat + Duration::minutes(CFG.jwt.jwt_exp);
    let claims = Claims {
        user_id: payload.user_id.to_owned(),
    ...
    // AuthBody::new(token, claims.exp, CFG.jwt.jwt_exp, token_id);
    AuthBody {
        token: token + &token_id,
        token_type: "Bearer".to_string(),
        exp: claims.exp,
        exp_in: CFG.jwt.jwt_exp,
    }
}
