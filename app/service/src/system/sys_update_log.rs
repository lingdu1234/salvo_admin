...
pub async fn add(
    ...
    sys_update_log::Entity::insert(insert_data)
        .exec(&txn)
        .await?;
...
pub async fn edit(
    ...
    sys_update_log::Entity::update_many()
        .col_expr(
            ...
            Expr::value(req.app_version.clone()),
        )
        .col_expr(
        ...
        .col_expr(
            sys_update_log::Column::Title,
            Expr::value(req.title.clone()),
        )
        .col_expr(
            sys_update_log::Column::Content,
            Expr::value(req.content.clone()),
        )
        .col_expr(sys_update_log::Column::UpdatedBy, Expr::value(user_id))
        .col_expr(
            sys_update_log::Column::UpdatedAt,
    ...
    C: ConnectionTrait + TransactionTrait,
{
    let s = sys_update_log::Entity::update_many()
        .col_expr(
            sys_update_log::Column::DeletedAt,
            Expr::value(Local::now().naive_local()),
        )
